...
- **片段**
	片段是客户端存储在分散存储网络中的数据的一部分。例如，可以将数据（可能是一个目录）有意地分为许多部分，并且每个部分可以由一组不同的存储矿工存储。
	
- **扇区**
	扇区是存储矿工提供给网络的一些磁盘空间（可以认为是与特定存储提供者的磁盘空间的特定部分相关联的唯一ID）。矿工将客户的物品存放在其所在的区域，并为其服务赚取代币。为了存储片段，存储矿工必须向网络保证其扇区可用。
